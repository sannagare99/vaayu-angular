class API::V2::DriversController < ApplicationController
  ...
  def create
    # business_associate_id = params[:business_associate_id].to_i if params[:business_associate_id].present?
    # user =  User.new
    # user.role = 3
    # set_driver_user_field(user,params)
    @driver = Driver.new(driver_params)
    # @driver.date_of_birth = params[:date_of_birth]
  ...
  def update
    if @driver.update(driver_params)
      render json: {status: "SUCCESS" , message: "UPDATE SUCCESS", data: @driver},status: :ok
    else
      render json: {status: "ERROR" , message: "UPDATE FAIL", data: @driver.errors},status: :unprocessable_entity
    end
  end
  ...
  def api_v1_version
    v2_version = request.url.scan(/v2/)
  end
  ...
  protected
   ...
   def upload_docs(params)
    ...
    def driver_params
      ...
      #   :site_id )
      params.permit(:business_associate_id, :licence_number, :driver_name, :alternate_number,:date_of_birth,:father_spouse_name,:gender, :blood_group, :licence_type, :licence_validity, :badge_number, :badge_expiry_date, :ifsc_code,:bank_name, :bank_no,:profile_picture_url,:driver_badge_doc_url,:driving_license_doc_url,:id_proof_doc_url,:driving_registration_form_doc_url )
    end
end