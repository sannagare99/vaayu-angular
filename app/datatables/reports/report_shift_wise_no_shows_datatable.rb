class Reports::ReportShiftWiseNoShowsDatatable
  ...
  def csv
    CSV.generate do |csv|
      ...
      get_trips.map { |info, trip| csv << Reports::ReportShiftWiseNoShowDatatable.new(info, trip).data.values }
    end
  end
  ...
  def data
    all_trips.map { |info, trip| Reports::ReportShiftWiseNoShowDatatable.new(info, trip).data }
  end
