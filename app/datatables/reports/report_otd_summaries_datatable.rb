class Reports::ReportOtdSummariesDatatable
  ...
  def csv
    CSV.generate do |csv|
      ...
      all_trips.each { |date, trips| csv << Reports::ReportOtdSummaryDatatable.new(date, trips).data.values }
    end
  end
  ...
  def data
    all_trips.map do |date, trips|
      Reports::ReportOtdSummaryDatatable.new(date, trips).data
    end
  end
  ...
  def get_trips
    # Trip.where(:trip_type => :check_out).where(:start_date => filter_params['startDate']..filter_params['endDate']).group_by(&:scheduled_data)
    Trip.joins(:trip_routes, :employee_trips)
        .select("trips.id,DATE(convert_tz(employee_trips.date,'-05:30', '+00:00')) as date,DATE_FORMAT(convert_tz(employee_trips.date,'-05:30', '+00:00'), '%H:%i') as shift_time,trips.status,trips.completed_date,trips.scheduled_date,trips.scheduled_approximate_duration,employee_trips.date as employee_trip_date,trips.cancel_status")
