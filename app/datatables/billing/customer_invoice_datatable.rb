class Billing::CustomerInvoiceDatatable
  ...
  def get_data
    ...
    if @trip_invoices.first&.trip_id.nil?
      trip = VehicleTripInvoice.where(:vehicle_id => @trip_invoices.first&.vehicle_id).where(:trip_invoice_id => @trip_invoices&.first&.id).first&.trip
