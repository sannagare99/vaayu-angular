class Billing::BaInvoicesDatatable
  ...
  def data
    all_invoices.map { |invoice| Billing::BaInvoiceDatatable.new(invoice).data }
  end
  ...
  def get_invoices
    BaInvoice.where(:date => filter_params['startDate']..filter_params['endDate']).order(id: :desc)
  end
