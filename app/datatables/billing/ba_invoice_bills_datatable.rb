class Billing::BaInvoiceBillsDatatable
  ...
  def data
    @all_data['vehicle_mapping'].map { |data_item| Billing::BaInvoiceBillDatatable.new(data_item).data}    
  ...
  def get_trip_invoices
    ...
    @trip_invoices.each do |trip_invoice|
      if !trip_invoice.trip_id.nil?        
        ...
        zone_name = trip_invoice&.ba_zone_rate&.name
        ...
        if vehicle_mapping[plate_number + '-' + zone_name].blank?
          vehicle_mapping[plate_number + '-' + zone_name] = {
            ...
            'zone_name' => zone_name,
            'total_trips' => 1,            
            'rate' => trip_invoice.ba_zone_rate.rate,
            'guard_rate' => trip_invoice.ba_zone_rate.guard_rate,
            'hours_on_duty' => 0,
            'mileage_on_duty' => 0,
            'hours_on_trips' => 0,
            'mileage_on_trips' => 0,
          ...
          vehicle_mapping[plate_number + '-' + zone_name] = {   
            ...
            'rate' => vehicle_mapping[plate_number + '-' + zone_name]['rate'] + trip_invoice.ba_zone_rate.rate,
            'guard_rate' => vehicle_mapping[plate_number + '-' + zone_name]['guard_rate'] + trip_invoice.ba_zone_rate.guard_rate,
            'hours_on_duty' => 0,
            'mileage_on_duty' => 0,
            'hours_on_trips' => 0,
            'mileage_on_trips' => 0,
      ...
      #it is a package rate invoice
        ...
        zone_name = trip_invoice&.ba_zone_rate&.name
        trips = VehicleTripInvoice.where(:vehicle_id => trip_invoice.vehicle_id).where(:ba_trip_invoice_id => trip_invoice.id)
