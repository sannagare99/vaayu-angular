$(function () {
     ...
     * Init table
     */
    var table = '#operator-active-trips-table';
    var operatorActiveTripsTable = $();

    $('a[href="#operator-active-trips"]').on('shown.bs.tab', function (e) {
        if (loadedTabs['operator-active-trips']) return;

        // set loaded state
        loadedTabs['operator-active-trips'] = true;

        if (!loadedDatatables[table]) {

            operatorActiveTripsTable = $(table).DataTable({
                serverSide: true,
                ajax: {
                    url: "/trips",
                    data: {
     ...
     * Init map
     */
    var operatorActiveTripsMapID = 'map-operator-active-trips';

    if ($('#map-operator-active-trips').length) {

        maps[operatorActiveTripsMapID] = Gmaps.build('Google', {markers: {clusterer: undefined}});

        maps[operatorActiveTripsMapID].buildMap({
            internal: {
                id: operatorActiveTripsMapID
            },
    ...
    $(table).on('click', '.map-picker-ico a', {
        mapId: operatorActiveTripsMapID,
        table: table
