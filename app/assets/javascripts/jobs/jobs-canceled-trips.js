$(function () {
     ...
     * Init table
     */
    var table = '#operator-canceled-trips-table';
    var operatorCancelTripsTable = $();

    $('a[href="#operator-canceled-trips"]').on('shown.bs.tab', function (e) {
        if (loadedTabs['operator-canceled-trips']) return;

        // set loaded state
        loadedTabs['operator-canceled-trips'] = true;

        if (!loadedDatatables[table]) {

            operatorCancelTripsTable = $(table).DataTable({
                serverSide: true,
                ajax: {
                    url: "/trips",
                    data: {
     ...
     * Init map
     */
    var operatorCancelTripsMapID = 'map-operator-canceled-trips';

    if ($('#map-operator-canceled-trips').length) {

        maps[operatorCancelTripsMapID] = Gmaps.build('Google', {markers: {clusterer: undefined}});

        maps[operatorCancelTripsMapID].buildMap({
            internal: {
                id: operatorCancelTripsMapID
            },
    ...
    $(table).on('click', '.map-picker-ico a', {
        mapId: operatorCancelTripsMapID,
        table: table
