$(function () {
     ...
     * Init table
     */
    var table = '#employee-active-trips-table';
    var employeeActiveTripsTable = $();

    $('a[href="#employee-active-trips"]').on('shown.bs.tab', function (e) {
        if (loadedTabs['employee-active-trips']) return;

        // set loaded state
        loadedTabs['employee-active-trips'] = true;

        if (!loadedDatatables[table]) {

            employeeActiveTripsTable = $(table).DataTable({
                serverSide: true,
                ajax: {
                    url: "/trips",
                    data: {
                ...
                initComplete: function () {
                    loadedDatatables[table] = true;
                     ...
                     $('#active-trips-count').text("Total Active Trips: " + info.recordsTotal);
                }
            });
     ...
     * Init map
     */
    var employeeActiveTripsMapID = 'map-employee-active-trips';

    if ($('#map-employee-active-trips').length) {
        maps[employeeActiveTripsMapID] = Gmaps.build('Google', {markers: {clusterer: undefined}});

        maps[employeeActiveTripsMapID].buildMap({
            internal: {
                id: employeeActiveTripsMapID
            },
    ...
    $(table).on('click', '.map-picker-ico a', {
        mapId: employeeActiveTripsMapID,
        table: table
