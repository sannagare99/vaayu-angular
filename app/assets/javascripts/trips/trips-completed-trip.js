$(function () {
     ...
     * Init table
     */
    var table = '#employee-completed-trips-table';
    var employeeCompletedTripsTable;

    $('a[href="#employee-completed-trips"]').on('shown.bs.tab', function (e) {

        if (!loadedDatatables[table]) {

            employeeCompletedTripsTable = $(table).DataTable({
                destroy: true,
                serverSide: true,
                ajax: {
                    url: "/trips",
     ...
     * Init map
     */
    var employeeTrRostersMapID = 'map-employee-completed-trips';

    if ($('#map-employee-completed-trips').length) {
