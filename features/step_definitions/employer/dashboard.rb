...
Given(/^I create data for daily macro parameters in the employer dashboard$/) do
  step 'Create operator for the employer dashboard'
  step 'Create site for employer dashboard'
  step 'Create employees for employer dashboard'
  step 'Create vehicle for employer dashboard'
  step 'Create driver for employer dashboard'
  step 'Create daily employee trips for employer dashboard'
  step 'Create daily trips for employer dashboard'
  step 'Create daily driver shifts trips data for employer dashboard'
  step 'Create daily trip route exceptions data for employer dashboard'
end

Given(/^Create operator for the employer dashboard$/) do
  @operator = FactoryGirl.create(:operator, logistics_company: @logistics_company)
end

Given(/^Create site for employer dashboard$/) do
...
Given(/^Create employees for employer dashboard$/) do
...
Given(/^Create daily employee trips for employer dashboard$/) do
...
Given(/^Create daily trips for employer dashboard$/) do
...
Given(/^Create daily driver shifts trips data for employer dashboard$/) do
  DriversShift.create(driver_id: @driver_1.id, vehicle_id: @vehicle_1.id, start_time: 3.hours.ago, end_time: 3.hours.from_now)
end

Given(/^Create daily trip route exceptions data for employer dashboard$/) do
...
Then(/^I can see macro parameters in the employer dashboard$/) do
  # step 'I can see exceptions macro parameter of employer dashboard'
  step 'I can see completed trips macro parameter of employer dashboard'
  step 'I can see on time arrivals macro parameter of employer dashboard'
  step 'I can see fleet utilization macro parameter of employer dashboard'
  step 'I can see employee satisfation macro parameter of employer dashboard'
end

Then(/^I can see exceptions macro parameter of employer dashboard$/) do
...
Then(/^I can see completed trips macro parameter of employer dashboard$/) do
  # Total Trips
  page.all('.count')[0].text.should eq('1')
  # Distance per Trip
  # page.all('.count')[7].all('span')[0].text.should eq('24')
  # Duration per trip
  page.all('.count')[8].all('span')[0].text.should eq('60')
  # Employee Check In
  page.all('.count')[11].text.should eq('100%')
  # Manifest Fulfilment
  page.all('.count')[13].text.should eq('100%')
  ...
  # parameters[0].text.should eq('24 km Total Distance')
  # distance_strings = parameters[1].text.split(' ')
  # distance_strings[0].to_i.should eq(24)
  # parameters[2].text.should eq('1 h 0 m Duration per Trip')
end

Then(/^I can see on time arrivals macro parameter of employer dashboard$/) do
  # On time arrival
  page.all('.count')[1].text.should eq('100%')
  # On Time Departure
  page.all('.count')[3].text.should eq('0%')
  # Average Delay
  # page.all('.pull-right')[1].text.should eq('0.37 min')
  # Total Delayed Trips
  page.all('.pull-right')[2].text.should eq('1')
  # Total Logins Affected
  page.all('.pull-right')[3].text.should eq('1')
...
Then(/^I can see fleet utilization macro parameter of employer dashboard$/) do
  # Fleet utilisation
  page.all('.count')[5].text.should eq('20%')
  page.all('.col-md-5')[0].text.should eq('5 seater')
  page.all('.col-md-3')[0].text.should eq('1.0')
  # Distance Per Day
  page.all('.pull-right')[4].text.should eq('24 km')
  # Total Logins Affected
  page.all('.pull-right')[5].text.should eq('1 min')
...
Then(/^I can see employee satisfation macro parameter of employer dashboard$/) do
  # Employee Satisfaction
  page.all('.count')[9].text.should eq('4.0')
...
Given(/^I create data for weekly and monthly macro parameters in the employer dashboard$/) do
  step 'Create operator for the employer dashboard'
  step 'Create site for employer dashboard'
  step 'Create employees for employer dashboard'
  step 'Create vehicle for employer dashboard'
  step 'Create driver for employer dashboard'
  step 'Create weekly and monthly employee trips for employer dashboard'
  step 'Create weekly and monthly trips for employer dashboard'
  step 'Create weekly and monthly driver shifts trips data for employer dashboard'
  step 'Create weekly and monthly trip route exceptions data for employer dashboard'
end

Given(/^Create weekly and monthly employee trips for employer dashboard$/) do
  @trip_time_1 = Time.now - 3.days
  @employee_trip_1 = EmployeeTrip.create(site_id: @site.id, employee_id: @employee_1.id, trip_type: 0, status: 'trip_created', state: 1, schedule_date: @trip_time_1, bus_rider: false, date: @trip_time_1, rating: 4)
end

Given(/^Create weekly and monthly trips for employer dashboard$/) do
  @trip_1 = Trip.create(site_id: @site.id, trip_type: 0, status: 'active', employee_trip_ids: [@employee_trip_1.id], driver: @driver_1, vehicle: @vehicle_1, start_date: @trip_time_1 - 15.minutes)
  @employee_trip_1.update(trip_id: @trip_1.id, trip_route_id: @trip_1.trip_routes.first.id)
end

Given(/^Create weekly and monthly driver shifts trips data for employer dashboard$/) do
  DriversShift.create(driver_id: @driver_1.id, vehicle_id: @vehicle_1.id, start_time: 5.days.ago, end_time: 3.hours.from_now)
end

Given(/^Create weekly and monthly trip route exceptions data for employer dashboard$/) do
  TripRouteException.create(trip_route_id: @trip_1.trip_routes.first.id, date: @trip_time_1, exception_type: :employee_no_show, status: :closed, resolved_date: @trip_time_1 - 5.minutes)
  @employee_trip_1.update(status: :completed)
  @trip_1.update(status: :completed, completed_date: @trip_time_1 - 1.minutes, real_duration: 1.minutes)
end
...
Then(/^I can see trip related micro parameters in the employer dashboard$/) do
  step 'I can see exceptions micro parameter of employer dashboard'
  step 'I can see completed trips micro parameter of employer dashboard'
  step 'I can see on time arrivals micro parameter of employer dashboard'
end

Then(/^I can see non-trip related micro parameters in the employer dashboard$/) do
  step 'I can see fleet utilization micro parameter of employer dashboard'
  step 'I can see employee satisfation micro parameter of employer dashboard'
end

Then(/^I can see exceptions micro parameter of employer dashboard$/) do
...
Then(/^I can see completed trips micro parameter of employer dashboard$/) do
  ...
  distance_parameters[0].find('strong').text.should eq('24 km')
...
Then(/^I can see on time arrivals micro parameter of employer dashboard$/) do
...
Then(/^I can see fleet utilization micro parameter of employer dashboard$/) do
...
Then(/^I can see employee satisfation micro parameter of employer dashboard$/) do
